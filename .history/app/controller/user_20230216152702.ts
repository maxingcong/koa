...
export const insertUser = async (params: {
    ...
    console.log(dayjs(Date.now()).format('YYYY-MM-DD hh:mm:dd'));

    const { code } = await insert(`insert into t_user 
    (user_name,password,is_admin,role,phone,create_time)
     values (${username},${intPassword},0,"${role}",${phone},${Date.now().toLocaleString()})`)
    // console.log(code);
    if (code !== 200) {
        data.code = -1
        data.msg = '创建失败'
