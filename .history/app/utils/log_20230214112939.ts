...
log4js.configure({
    ...
    appenders: {
        ...
        allLog: {
            type: 'file', filename: `./log/all/${dayjs(Date.now()).format('YYYY/MM/DD')}.log`,
            keepFileExt: true, maxLogSize: 10485760, backups: 3
        },
        //http请求日志  http请求日志需要app.use引用一下， 这样才会自动记录每次的请求信息
        httpLog: {
            type: "dateFile", filename: `./log/httpAccess/${dayjs(Date.now()).format('YYYY/MM/DD')}.log`,
            pattern: ".yyyy-MM-dd", keepFileExt: true
        },
        //错误日志 type:过滤类型logLevelFilter,将过滤error日志写进指定文件
        errorLog: { type: 'file', filename: `./log/${dayjs(Date.now()).format('YYYY/MM/DD')}.log` },
...
export const httpLogger = log4js.connectLogger(httpLog, { level: 'MARK' });
