...
export = {
    logInfo: function (info: any) {    //封装普通日志
        if (info) {
            consoleLogger.info(formatText.info(info));
        }
    },
    logResponse: function (ctx: any, resTime: any) {    //封装响应日志
        if (ctx) {
            resLogger.info(formatText.response(ctx, resTime));
        }
    },
    logHandle: function (res: any) {    //封装操作日志
        if (res) {
            handleLogger.info(formatText.handle(res));
        }
    },

    logError: function (ctx: any, error: any, resTime: any) {//封装错误日志
        if (ctx && error) {
