...
const createServer = async (): Promise<Koa> => {
    ...
    routesDir.forEach((dirPath: string) => {
        ...
        if (dirPath.split('.').length < 2) {
            instance[dirPath] = new KoaRouter({ prefix: `/web/api/${dirPath}` }); //设置 路由前缀
            ...
            // console.log(instance[dirPath].routes());
            routerList.push(instance[dirPath].routes());
            return
        }
        // instance[dirPath].use(require(path.resolve(`app/route/${dirPath}`)).default.routes());//读取文件  模块信息
        // routerList.push(instance[dirPath].routes());//
    });
